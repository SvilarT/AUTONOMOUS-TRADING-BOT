...
class MongoIndexServiceV2:
    ...
    async def ensure_indexes(self) -> Dict[str, Any]:
        ...
        await create(self.db.live_execution_sessions, [("token_hash", 1)], unique=True, name="uniq_live_execution_session_token")
        await create(self.db.live_execution_sessions, [("user_id", 1), ("created_at", -1)], name="live_execution_session_user_created")
        await create(self.db.live_execution_sessions, [("expires_at", 1)], expireAfterSeconds=0, name="live_execution_session_expiry_ttl")
        await create(self.db.live_rate_limit_events, [("user_id", 1), ("created_at", -1)], name="live_rate_limit_user_created")
        await create(self.db.live_rate_limit_events, [("request_ip", 1), ("created_at", -1)], name="live_rate_limit_ip_created")
        await create(self.db.live_rate_limit_events, [("created_at", 1)], expireAfterSeconds=3600, name="live_rate_limit_ttl")
        await create(self.db.live_idempotency_records, [("user_id", 1), ("idempotency_key", 1)], unique=True, name="uniq_live_idempotency_user_key")
        await create(self.db.live_idempotency_records, [("expires_at", 1)], expireAfterSeconds=0, name="live_idempotency_expiry_ttl")
        await create(self.db.live_execution_events, [("user_id", 1), ("event_type", 1), ("created_at", -1)], name="live_execution_event_user_type_created")
        await create(self.db.live_execution_events, [("created_at", 1)], expireAfterSeconds=2592000, name="live_execution_events_ttl")
        ...
        await create(self.db.manual_live_pilot_reports, [("user_id", 1), ("generated_at", -1)], name="manual_live_pilot_reports_user_generated")
        await create(self.db.manual_live_pilot_reports, [("user_id", 1), ("live_order_id", 1)], unique=True, name="uniq_manual_live_pilot_report_order")
        await create(self.db.manual_live_pilot_reports, [("report_hash", 1)], unique=True, sparse=True, name="uniq_manual_live_pilot_report_hash")
        await create(self.db.manual_live_pilot_signoffs, [("user_id", 1), ("signed_at", -1)], name="manual_live_pilot_signoffs_user_signed")
        await create(self.db.manual_live_pilot_signoffs, [("user_id", 1), ("live_order_id", 1)], unique=True, name="uniq_manual_live_pilot_signoff_order")
        await create(self.db.manual_live_pilot_signoffs, [("signoff_hash", 1)], unique=True, sparse=True, name="uniq_manual_live_pilot_signoff_hash")
