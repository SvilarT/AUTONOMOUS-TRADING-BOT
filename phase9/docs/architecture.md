...
* **Risk Manager** – Computes exposure, volatility, Value‑at‑Risk,
  ...
  breached (see `phase9/core/risk_manager.py`).
* **Portfolio Service** – Tracks cash balances, positions and
  P&L for each user or account.  In Phase 9 this uses an in‑memory
...
* **Marketplace Service** – Allows users to register, rate and
  ...
  reputation scores (`phase9/marketplace/service.py`).
* **Security & Compliance** – New modules in Phase 8/9 manage secrets
  (`phase9/security/secrets_manager.py`), simulate a custody service
  (`phase9/security/custody.py`) and stub out KYC and AML checks
  (`phase9/compliance/*`).
