This `phase9` directory contains the most feature‑complete version of the
autonomous trading bot to date.  Building on earlier phases, it introduces
comprehensive testing, documentation and a continuous integration (CI)
pipeline while preserving the modular architecture established in
Phase 2–8.
...
* **FastAPI Routes** – REST endpoints are organized by function
  (`/users`, `/marketplace`, `/security`, `/compliance`) and return
  Pydantic‑validated responses.
* **Testing Suite** – The `tests/` directory contains unit and
  integration tests for every major component.  Tests can be run with
  `pytest` and cover both synchronous and asynchronous code paths.
...
4. Explore the documentation in `phase9/docs/` for more details on
