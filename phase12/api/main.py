...
def create_app() -> FastAPI:
    ...
    async def startup_event() -> None:
        ...
        # Environment and feature flags from config
        env_cfg = Services.config.get("environment", {})
        Services.environment = Environment(mode=env_cfg.get("mode", "paper"))
        feat_cfg = Services.config.get("features", {})
        Services.feature_flags = FeatureFlagService(feat_cfg)
    ...
    async def submit_trade(user_id: str, req: TradeRequest, services: Services = Depends(get_services)) -> TradeResponse:
        ...
        try:
            ...
            if action == "BUY" and allowed.get("allowed"):
                ...
                # Broadcast trade event to live dashboard
                try:
                    await trade_notifier.broadcast(
                        {
                            "user_id": user_id,
                            "symbol": req.symbol,
                            "action": "BUY",
                    ...
                    # Ignore broadcast errors to avoid interrupting trade flow
                    pass
                ...
                # Compute the notional based on filled units and price
                notional_usd = (result.get("filled_price", price_map[req.symbol]) or price_map[req.symbol]) * units
                # Broadcast trade event to live dashboard
                try:
                    await trade_notifier.broadcast(
                        {
    ...
    # Include additional routers (marketplace, security, compliance, monitoring, notifications, live dashboard)
    ...
    # Live market dashboard and trade stream
    from .live_market import router as live_market_router
    app.include_router(live_market_router)

    # Environment and feature flags introspection
    @app.get("/environment")
    def get_environment(services: Services = Depends(get_services)) -> Dict[str, str]:
        """Return the current operating environment."""
        if services.environment is None:
            raise HTTPException(status_code=503, detail="Environment not initialised")
        return {"mode": services.environment.mode}

    @app.get("/features")
    def get_features(services: Services = Depends(get_services)) -> Dict[str, Dict[str, bool]]:
        """Return the set of configured feature flags."""
        if services.feature_flags is None:
            raise HTTPException(status_code=503, detail="Feature flags not initialised")
        return {"flags": services.feature_flags.all_flags()}
