This `phase11` directory contains the most feature‑complete version of the
...
The architecture remains modular, allowing new features to be integrated
...
2. Run the unit tests:
   ...
   ```bash
   pytest -q phase11/tests
   ```

3. Launch the FastAPI application for interactive exploration:

   ```bash
   uvicorn phase11.api.main:app --reload
   ```

4. Explore the documentation in `phase11/docs/` for more details on
   system architecture, environment configuration and staged rollout
   features.
