...
* **Risk Manager** – Computes exposure, volatility, Value‑at‑Risk,
  ...
  breached (see `phase11/core/risk_manager.py`).
* **Portfolio Service** – Tracks cash balances, positions and
  P&L for each user or account.  In Phase 11 this uses an in‑memory
...
* **Marketplace Service** – Allows users to register, rate and
  ...
  reputation scores (`phase11/marketplace/service.py`).
* **Security & Compliance** – Modules from Phase 8 manage secrets
  (`phase11/security/secrets_manager.py`), simulate a custody service
  (`phase11/security/custody.py`) and stub out KYC and AML checks
  (`phase11/compliance/*`).
* **Monitoring & Notifications** – Phase 10 added Prometheus metrics and
  a lightweight notification system.  Metrics are defined in
  `phase11/monitoring/metrics.py` and exposed via the API in
  `phase11/api/monitoring.py`.  The notification service lives in
  `phase11/notifications/service.py` and routes for sending
  notifications are defined in `phase11/api/notifications.py`.

* **Environment & Feature Flags** – Phase 11 introduces an
  ``Environment`` class (`phase11/environment.py`) that encodes whether the
  bot is running in paper mode or live mode, and a
  ``FeatureFlagService`` (`phase11/feature_flags.py`) that stores
